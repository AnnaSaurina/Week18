...
const heroesArrJSON = JSON.stringify(heroesArr);
...
document.addEventListener('heroesPage', () => {
    ...
    if (localStorage.getItem(saveName) === name) {
      const savePoint = localStorage.getItem(saveName);
      console.log(savePoint);
    }

    let rating = document.querySelectorAll('.hero_card-points-input');
    let ratingArr = Array.from(rating);

    ratingArr.forEach(function (item) {
      item.addEventListener('click', () => {
        saveEstimation = item.ariaValueMax;
        saveName = item.name;
        console.log(item.nam, item.value);
        localStorage.setItem(saveNamem, saveEstimation);
      })
    })
});